  ...
  const getFileNames = async () => {
    try {
      const response = await fetch("http://192.168.1.9", {
        ...
        // heade/ccess-Control-Allow-Credentials', 'true');
        // // body: JSON.stringify({ ids }),
      });
      ...
      if (response) {
        ...
        //     // Check if the item has a "folder" property
        //     if (item.folder) {
        //         console.log(`Folder: ${item.folder}`);
        //     } else if (item.file) {
        //         // Check if the item has a "file" property
  ...
  return (
    <div className="p-4 sm:ml-64 mt-14">
      ...
      <div className="flex my-3 items-center justify-end">
        ...
        />
        <button onClick={getFileNames} class="mx-4 text-white bg-blue-700 hover:bg-blue-800 font-medium rounded-lg text-md px-5 py-2 text-center">
          Refresh
        </button>
      </div>
